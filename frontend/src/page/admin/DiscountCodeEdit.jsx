...
const DiscountCodeEdit = () => {
  ...
  useEffect(() => {
    const fetchDiscountCode = async () => {
      try {
        const res = await discountCodeService.readDiscountCodeById(id);
        const data = res.data;
        // Format date to YYYY-MM-DD for input type="date"
        let formattedDate = "";
        if (data.expirationDate) {
          formattedDate = data.expirationDate.substring(0, 10);
        }
        ...
        setForm({
          ...
          status: data.status !== undefined ? data.status.toString() : "true",
          description: data.description || "",
        });
      } catch (error) {
        console.error("Lỗi khi lấy thông tin mã giảm giá", error);
        toast.error("Không thể lấy thông tin mã giảm giá");
      }
    };
    if (id) {
      fetchDiscountCode();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await discountCodeService.updateDiscountCode(id, submitData);
      toast.success("Cập nhật mã giảm giá thành công");
      navigate("/admin/discount-code");
    } catch (error) {
      console.error("Lỗi khi cập nhật mã giảm giá", error);
      const msg = error.response?.data?.message || "Cập nhật mã giảm giá thất bại";
      toast.error(msg);
    }
  };
